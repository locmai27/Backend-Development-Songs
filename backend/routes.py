...
def count():
    return jsonify({"count": db.songs.count()}), 200

@app.route("/songs", methods=["GET"])
def get_songs():
    songs = db.songs.find()
    #send back as a list of songs
...
def create_song(id):
    ...
    if db.songs.find_one({"_id": ObjectId(id)}):
        return {"Message": f"song with id {id} already present"}, 302
    song["_id"] = ObjectId(id)
...
def update_song(id):
    song = request.get_json()
    if db.songs.find_one({"_id": ObjectId(id)}):
        db.songs.update_one({"_id": ObjectId(id)}, {"$set": song})
